...
void Lab4::RenderScene() {
    ...
    modelMatrix *= transform3D::Translate(-2.5f, -0.5f, 1.5f);
    modelMatrix *= transform3D::RotateOY(myAngularStep);
    modelMatrix *= transform3D::Translate(2.5f, 0.5f, -1.5f);
...
void Lab4::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_I)
        miniViewportArea.y += 1.65f;
    if (key == GLFW_KEY_J)
        miniViewportArea.x -= 1.65f;
    if (key == GLFW_KEY_K)
        miniViewportArea.y -= 1.65f;
    if (key == GLFW_KEY_L)
        miniViewportArea.x += 1.65f;
    if (key == GLFW_KEY_U) {
        miniViewportArea.width /= 1.3f;
        miniViewportArea.height /= 1.3f;
    }
    if (key == GLFW_KEY_O) {
        miniViewportArea.width *= 1.3f;
        miniViewportArea.height *= 1.3f;
